namespace :dates do
  ...
  task :benchmark, [:sample_size] => [:environment] do |task, args|
    ...
    summaries.each do |model, sets|
      sets.each do |set, summary|
        puts "%-28s %-8s %8.1f%% %5d/%-3d %3d/%-2d %9.1f" % [
          model, set, summary[:accuracy], summary[:correct], summary[:captions],
          summary[:json], summary[:captions], summary[:seconds_per_caption]
        ]
      end
    end
  end
